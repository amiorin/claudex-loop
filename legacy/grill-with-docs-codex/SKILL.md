...
description: 'Legacy docs-aware plan-hardening workflow, superseded by /claudex-loop. Claude interviews the user while checking the project glossary, domain model, code, CONTEXT.md, and ADRs; updates living documentation as decisions settle; then writes PLAN.md and iterates adversarial OpenAI Codex reviews in the same session. Use only when the user explicitly asks for "/grill-with-docs-codex", "grill me against the docs then have codex review", or "stress-test this against our domain model then get a second model on it". Prefer /claudex-loop for new work. Do not use for existing-code reviews or trivial changes.'
---
