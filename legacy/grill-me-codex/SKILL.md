---
name: grill-me-codex
description: 'Legacy two-act plan-hardening workflow, superseded by /claudex-loop. Claude first interviews the user one question at a time until the decision tree is resolved, then writes PLAN.md and iterates adversarial OpenAI Codex reviews in the same session until approval or MAX_ROUNDS. Use only when the user explicitly asks for "/grill-me-codex", "grill me then have codex review", "grill me and stress-test the plan", or "interview me about this plan then get a second model on it". Prefer /claudex-loop for new work, /grill-with-docs-codex for the legacy docs-aware variant, and /codex-review when a plan already exists. Do not use for existing-code reviews or trivial changes.'
---
