---
name: codex-build
description: 'Delegate implementation of a frozen PLAN.md or other locked spec to OpenAI Codex with full write access while Claude reviews the complete diff, runs the proof command, and iterates fixes in the same Codex session. The human approves before any commit. Use when the user says "/codex-build", "have codex build this", "codex implement the plan", "hand the plan to codex", or "delegate the build to codex"; after a plan survives /claudex-loop or /codex-review and the user chooses Codex; or for well-specified refactors, migrations, reproducible bug fixes, and test work. Do not use for tiny edits, unresolved design work, reviews of existing code, or work requiring Claude-session-only tools such as MCP, secrets, or a browser.'
---
