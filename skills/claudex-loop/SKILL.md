---
name: claudex-loop
description: 'Four-phase plan-hardening workflow: recon the codebase or greenfield landscape and draft an assumptions ledger; interrogate only load-bearing decisions; have OpenAI Codex adversarially review and iterate the locked PLAN.md; optionally build with one model and cross-inspect with the other. Use when the user says "/claudex-loop", "claudex this", "run the claudex loop", "/crucible", "put this through the crucible", "crucible this plan", "grill me then have codex review", or "stress-test this plan before we build". Also use before high-stakes work involving auth, schemas, concurrency, migrations, payments, or greenfield architecture when the user wants alignment and cross-model review. Legacy /crucible triggers remain supported. For an already-locked plan needing only Codex review, use /codex-review; for reviewing existing code, use /codex:review. Do not use for trivial changes.'
---
